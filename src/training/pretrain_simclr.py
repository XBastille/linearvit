"""
SimCLR Contrastive Pretraining on unlabelled CMS data.

Learns instance-discriminative features by maximizing agreement
between two augmented views of the same image via NT-Xent loss.

Features:
  - Auto-resume from weights/pretrain_simclr_latest.pth
  - Early stopping with configurable patience
  - tqdm progress bars
  - Saves backbone weights compatible with existing finetune pipeline
...
def pretrain_simclr(config):
    ...
    model = LinearViT(
        ...
        num_classes=2,  # placeholder, heads not used
    ...
    print(f"SimCLR pretraining complete. Best val loss: {best_val_loss:.4f}", flush=True)
